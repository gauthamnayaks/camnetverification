...
#end#
formula update_bw#Nc# = max(minimum_bw, min(maximum_bw, 100#for i=1:Nc-1# - update_bw#i##end#));  // camera #Nc# gets the remaining percentage, NOTE: improtant update_bw1 and not bw1
...
formula framesize#i#_dist = min(maximum_framesize, max(minimum_framesize, ceil((q#i#+50) * maximum_framesize / 100)));
#end#
...
module c#i#
  ...
  [bw_allocated] (cam#i# = cam#i#_ready) -> 0.7 : (cam#i#' = cam#i#_thr_check) & (s#i#'= framesize#i#) + 0.3 : (cam#i#' = cam#i#_thr_check) & (s#i#'= framesize#i#_dist);
...
[bw_allocated] (cam#Nc# = cam#Nc#_ready) -> 0.7 : (cam#Nc#' = cam#Nc#_thr_check) & (s#Nc#'= framesize#Nc#) + 0.3:(cam#Nc#' = cam#Nc#_thr_check) & (s#Nc#'= framesize#Nc#_dist);
