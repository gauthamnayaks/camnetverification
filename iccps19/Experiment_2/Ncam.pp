...
#for i=1:Nc#
formula framesize#i# = min(maximum_framesize, max(minimum_framesize, ceil((q#i#+q#i#_dist) * maximum_framesize / 100)));
#end#
...
module c#i#
  ...
  [] (cam#i# = cam#i#_calc_fr) & (ceil(s#i# / bytes_millis)<= t#i#)-> 1 : (cam#i#' = cam#i#_fr_sent) & (e#i#' = t#i#) & (alloc#i#' = compute_tn1);
  [] (cam#i# = cam#i#_calc_fr) & (ceil(s#i# / bytes_millis) > t#i#)-> 1 : (cam#i#' = cam#i#_fr_drop) & (e#i#' = t#i#)  & (alloc#i#' = compute_tn1);
...
[] (cam#Nc# = cam#Nc#_calc_fr) & (ceil(s#Nc# / bytes_millis)<= t#Nc#) -> 1 : (cam#Nc#' = cam#Nc#_fr_sent) & (e#Nc#' = t#Nc#) & (alloc#Nc#' = compute_tn1);
[] (cam#Nc# = cam#Nc#_calc_fr) & (ceil(s#Nc# / bytes_millis) > t#Nc#) -> 1 : (cam#Nc#' = cam#Nc#_fr_drop) & (e#Nc#' = t#Nc#)   & (alloc#Nc#' = compute_tn1);
...
// ******************** CAMERA MODULES (END) ********************
 ...
 const env_ready = 0;
 const env_dist_calc = 1;

module environment
  env : [env_ready..env_dist_calc] init env_dist_calc;
  #for i=1:Nc#
    q#i#_dist:[0..50] init 0;
  #end#

  [last_cam_sent](env = env_ready)-> 1:(env'=env_dist_calc) & #for i=1:Nc-1#(q#i#_dist'=0) & #end# (q#Nc#_dist'=0);
  [last_cam_sent](env = env_ready) -> 1:(env'=env_dist_calc) & #for i=1:Nc-1#(q#i#_dist'=0) & #end# (q#Nc#_dist'=20);
  [last_cam_sent](env = env_ready) -> 1:(env'=env_dist_calc) & #for i=1:Nc-1#(q#i#_dist'=10) & #end# (q#Nc#_dist'=30);
  //[last_cam_sent](env = env_ready) -> 1:(env'=env_dist_calc) & #for i=1:Nc-1#(q#i#_dist'=20) & #end# (q#Nc#_dist'=50);
  //[last_cam_sent](env = env_ready) -> 1:(env'=env_dist_calc) & #for i=1:Nc-1#(q#i#_dist'=40) & #end# (q#Nc#_dist'=20);
  //[last_cam_sent](env = env_ready) -> 1:(env'=env_dist_calc) & (q1_dist' = 0) & (q2_dist' = 10);
  //[bw_allocated](env = env_ready) -> (env'=env_dist_calc) & (q1_dist' = 10) & (q2_dist' = 10);
  //[bw_allocated](env = env_ready) -> (env'=env_dist_calc) & (q1_dist' = 20) & (q2_dist' = 0);
  //[bw_allocated](env = env_ready) -> (env'=env_dist_calc) & (q1_dist' = 0) & (q2_dist' = 20);
  //[bw_allocated](env = env_ready) -> (env'=env_dist_calc) & (q1_dist' = 20) & (q2_dist' = 10);
  //[bw_allocated](env = env_ready) -> (env'=env_dist_calc) & (q1_dist' = 10) & (q2_dist' = 20);
  //[bw_allocated](env = env_ready) -> (env'=env_dist_calc) & (q1_dist' = 20) & (q2_dist' = 20);

  [bw_allocated] (env = env_dist_calc) & (!end) -> (env'= env_ready);
